...
## Scope — Gen4 v0.5.x Production Scale (docs only) — 2026-09-19

Docs-only scope on `05ea0df3` / v0.4.9 after PR #37. No product patch. Package
stays **0.4.9**. Needle **OFF**. Caps **not** raised. RW-058–088 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened.

Gen3 themes 1–3 are **complete (scripted)**. Live E1–E3 confirmation is
**deferred** (NIM Class C paused RW-084; OpenRouter free paused RW-086
429). Residual Class B (fallback PLAN, incomplete package, free-model
thrash) is input to Gen4 — not a fourth Gen3 theme.

Gen4 (Production Scale) is **planned / scoped**, not started as a build.
Recommended first **v0.5.0** candidate **G4-1** (live multi-provider /
free-provider production doctrine) is listed in [ROADMAP.md](ROADMAP.md)
— **not accepted**, **not implemented**. G4-2 (fallback/LLM plan quality),
G4-3 (operational scale), and G4-4 (extensibility; already in tree) remain
candidates. Next product work waits for a written accept. Class C is
**unblock the environment**, not a product patch.
...
## Gen4 / v0.5.0 — Live multi-provider Class C doctrine (RW-089) — 2026-09-19

Investigate-first confirmed the Cycle 24 hole: router failover existed, but
recovery re-read 403/429 as MODEL/TRANSIENT product retries (RW-084 three
MODEL_FAILURE attempts; RW-086 mid-run 429 burned tools). G4-1 is
**accepted**. Smallest patch: classify Class C **before** MODEL; 429 →
`RATE_LIMIT` ask_user; router skips Class-C-failed brains and rotates
preferred free; `free_lock` never silently spends paid. Package
**0.4.9 → 0.5.0**. Control plane preserved. Needle **OFF**. Caps **not**
raised. RW-058–088 facts are **not rewritten**.

### F-20260919-52 — 403/429 Class C pause + free-first rotation (RW-089)

| field | value |
|---|---|
| class | **capability** (Gen4 G4-1). Investigate-first: control-plane hole in *classification / pause*, not a missing router. Class C stays Class C. Not a re-open of live RW-086 artifact quality |
| status | **shipped in v0.5.0** |
| found in | RW-084 / F-20260919-46 (NIM HTTP 403); live RW-086 / F-20260919-49 (OpenRouter HTTP 429); ROADMAP G4-1 (PR #38 listed, this change accepts) |
| fixed in | **v0.5.0** — `class_c_kind` / recovery Class C before MODEL; `RATE_LIMIT` ask_user; router Class C skip + rotate; doctor doctrine. F-17 check-less / caps unchanged |
| objective / test | `tests/test_provider_class_c.py`; RW-089 |
| expected | 403 → AUTH ask_user; 429 → RATE_LIMIT ask_user; attempts=1; 0 Repair; free 429 rotates; free_lock never calls paid; 503 still retries |
| actual | scripted **PASS** (see RW-089). Live text_analyzer@12 **not** claimed |
...
## Scope — G4-2 live-gate resume / provider health (docs only) — 2026-09-19

Docs-only scope on `d194b8ff` / v0.5.0 after PR #39. No product patch. Package
stays **0.5.0**. Needle **OFF**. Caps **not** raised. RW-058–089 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened.
Do **not** invent a Class A patch for 403/429.

G4-1 is **shipped** (v0.5.0 / RW-089). Investigate-first: the next
production-scale hole is not residual Class B plan quality. RW-084
`rad doctor` providers **READY** while `GET /v1/models` **200** and
`chat/completions` **403**. RW-086 working-then-**429** with no durable
last-Class-C surface. Recommended next **v0.5.1** candidate **G4-2**
(live-gate resume / provider health observability) is listed in
[ROADMAP.md](ROADMAP.md) — **not accepted**, **not implemented**. Cycle
24's G4-2 (fallback plan quality) is now **G4-5**. G4-3 / G4-4 remain
later candidates. Next product work waits for a written accept. Class C
is **unblock the environment**, not a product patch.
...
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **IN PROGRESS** — G4-1 shipped; G4-2 listed, not accepted |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Scope — G4-3 live-use campaign / operator workflow (docs only) — 2026-09-19

Docs-only scope on `2d4c8b6d` / v0.5.1 after PR #41. No product patch. Package
stays **0.5.1**. Needle **OFF**. Caps **not** raised. RW-058–091 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened.
Do **not** invent a Class A patch for 403/429.

G4-2 is **shipped** (v0.5.1 / RW-090 / RW-091). Investigate-first: the next
production-scale hole is not residual Class B plan quality and not a
cost dashboard. G4-1 closed pause; G4-2 closed safe resume; the loop’s
next step is **use**. E1–E3 and G4-1/G4-2 have never been live-confirmed
together (last working inference: RW-086 on v0.4.7). Leftover
operator-workflow hole: online `rad doctor` re-pings chat while last
Class C still blocks; resume already skips (RW-091). Recommended next
**v0.5.2** candidate **G4-3** (live-use campaign / operator workflow)
is listed in [ROADMAP.md](ROADMAP.md) — **not accepted**, **not
implemented**. Cycle 26’s G4-3 (longer workloads + cost/budget) splits:
*use* keeps **G4-3**; cost/budget reporting is now **G4-6**. G4-4 / G4-5
remain later candidates. Next product work waits for a written accept.
Class C is **unblock the environment**, not a product patch.

| gate | result |
|---|---|
| `python3 -m pytest -q` | **596 passed** in 12.03s |
| `rad doctor --offline` | READY — 20 READY · 0 WARNING · 3 OPTIONAL · 0 ERROR (`RAD_HOME=/tmp/rad-g43-scope-gate`) |
| `rad acceptance` | **50/50 PASSED** (`/tmp/rad-g43-scope-gate/acceptance/20260919-062646_gate.json`) |
| `rad realworld` | **10 passed / 1 BLOCKED / 0 failed** (`live_nim` BLOCKED; `/tmp/rad-g43-scope-rw/realworld/20260919-062643_realworld.json`) |
| Package | **0.5.1** (no bump; no v0.5.2) |
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **IN PROGRESS** — G4-1 / G4-2 shipped; G4-3 listed, not accepted |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Scope — G4-5 fallback / LLM plan quality (docs only) — 2026-09-19

Docs-only scope on `d534bd30` / v0.5.2 after PR #43 and GitHub Release
**Version 5** / tag **v0.5.2**. No product patch. Package stays **0.5.2**.
Needle **OFF**. Caps **not** raised. RW-058–093 facts are **not rewritten**.
F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened. Do **not** invent
a Class A patch for 403/429. **No per-slice GitHub Release.**

**Version 5 pack** shipped (G4-1 / G4-2 / G4-3). Investigate-first: the
next product hole is not MCP, not a cost dashboard, and not live
confirmation of Version 5 as a package theme (G4-3 playbook already
ships; gates remain paused). Residual Class B PLAN `source=fallback`
(RW-085 / RW-086) is the measured gap. Recommended next **v0.5.3**
candidate **G4-5** (fallback / LLM plan quality under tight budgets) is
listed in [ROADMAP.md](ROADMAP.md) — **not accepted**, **not
implemented**. G4-4 stays not-next (no measured hole). G4-6 waits on
live use data. Next product work waits for a written accept. Class C is
**unblock the environment**, not a product patch.
...
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **IN PROGRESS** — Version 5 pack shipped; G4-5 listed, not accepted |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Gen4 / v0.5.3 — Fallback / LLM plan quality (RW-094 / RW-095) — 2026-09-19

Generation 4 production scale, **G4-5**. Package **0.5.2 → 0.5.3**.
Control plane preserved (models propose / RAD decides). Needle **OFF**.
Caps **not** raised (`max_plan_tasks` 16, `Budget.tool_calls` 60).
RW-058–093 facts are **not rewritten**. F-17 / F-18 / F-21 / F-26 stay
closed. A1 not reopened. Do **not** invent a Class A patch for 403/429.
**No GitHub Release / tag** — Version 5 packing doctrine: merge-only;
pack again only when Sanath asks.

Product: near-JSON LLM plans (markdown fences, trailing commas, a
top-level tasks array, extra braces after the first object) are recovered
as `source=llm` with machine checks. Coding-goal misses retry with a
compact JSON skeleton (`PLAN_CODING_RETRY`) instead of re-sending the
full prompt. Exhausted retries still `_fallback(obj)`: goal-only clause
split, cap 7, **no checks** on fallback *tasks* (F-17). Does **not**
claim live text_analyzer@12 now PASS.

### F-20260919-55 — near-JSON / compact coding retry (RW-094 / RW-095)

| field | value |
|---|---|
| class | **B** residual (live fallback PLAN quality on RW-085 / RW-086) addressed as **capability** (Gen4 G4-5). Not a re-open of F-17. Not Class A for 403/429 |
| status | **shipped in v0.5.3** |
| found in | live RW-085 / F-20260919-47 and live RW-086 / F-20260919-49 (`source=fallback` attempts=2; 4 clause-carved tasks); scoped Cycle 30 / PR #44 |
| fixed in | **v0.5.3** — `_json_obj` recovers fenced / trailing-comma / tasks-array / first-balanced JSON (RW-094); coding-goal retries use `PLAN_CODING_RETRY` (RW-095); `_fallback` unchanged (F-17 check-less / cap 7 / goal-only). Caps unchanged. Needle OFF |
| lane | scripted planner + controller (no NIM / no OpenRouter) |
| objective / test | `tests/test_llm_plan_quality.py`; RW-094 / RW-095 |
| disk | n/a (plan-time). Scripted: fenced/trailing/array JSON → `source=llm` coding graph with checks; timeout then compact retry → `source=llm`; exhausted → `source=fallback` 4 clause tasks, no checks; `DONE:` is not VERIFIED |
| expected | coding-goal near-JSON / compact retry is a structured plan, not clause-carve; exhausted fallback stays honest; false DONE **0** |
| actual | scripted **PASS** (see RW-094 / RW-095). Live text_analyzer@12 **not** claimed |
| notes | Live RW-085/086 remain Class B on free-model quality and historical fallback. This PR does **not** claim live PASS. A1 not reopened. E1–E3 live still **not confirmed**. F-17 stays closed. Needle off. Caps unchanged. **No release / tag.** |
...
## Scope — G4-7 coding artifact completeness (docs only) — 2026-09-19

Docs-only scope on `b8b67a43` / v0.5.3 after PR #45. No product patch. Package
stays **0.5.3**. Needle **OFF**. Caps **not** raised. RW-058–095 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened. Do
**not** invent a Class A patch for 403/429. **No per-slice GitHub Release.**

**G4-5** shipped (v0.5.3 / RW-094 / RW-095). Investigate-first: the next
product hole is not MCP, not a cost dashboard, and not live
confirmation of Version 5 / G4-5 as a package theme (G4-3 playbook
already ships; gates remain paused). Residual Class B **incomplete
package artifacts** (RW-081 empty `{}` / SyntaxError test; RW-085 / RW-086
missing README/tests + alt-schema summary) is the measured gap.
`infer_coding_checks` omits `file_exists` for named non-JSON package
files and `json_field` for named keys. Recommended next **v0.5.4**
candidate **G4-7** (coding artifact completeness / named package-file
contracts) is listed in [ROADMAP.md](ROADMAP.md) — **not accepted**,
**not implemented**. G4-4 stays not-next (no measured hole). G4-6 waits
on live use data. **G4-4 / G4-6 ids kept.** Next product work waits for
a written accept. Class C is **unblock the environment**, not a product
patch.
...
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **IN PROGRESS** — G4-5 shipped as v0.5.3; G4-7 listed, not accepted |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Gen4 / v0.5.4 — Coding artifact completeness (RW-096 / RW-097) — 2026-09-19

Generation 4 production scale, **G4-7**. Package **0.5.3 → 0.5.4**.
Control plane preserved (models propose / RAD decides). Needle **OFF**.
Caps **not** raised (`max_plan_tasks` 16, `Budget.tool_calls` 60).
RW-058–095 facts are **not rewritten**. F-17 / F-18 / F-21 / F-26 stay
closed. A1 not reopened. Do **not** invent a Class A patch for 403/429.
**No GitHub Release / tag** — Version 5 packing doctrine: merge-only;
pack again only when Sanath asks.

Product: coding-goal *objective_checks* now contract named package files
(`file_exists` for README.md / analyzer.py / main module) and named JSON
keys (`json_field` so empty `{}` / alt-schema is not VERIFIED). Fallback
*tasks* stay check-less (F-17). Does **not** claim live text_analyzer@12
now PASS.

### F-20260919-56 — named package-file / JSON-field objective contracts (RW-096 / RW-097)

| field | value |
|---|---|
| class | **B** residual (incomplete package artifacts on RW-081 / RW-085 / RW-086) addressed as **capability** (Gen4 G4-7). Not a re-open of F-17. Not Class A for 403/429 |
| status | **shipped in v0.5.4** |
| found in | live RW-081 / F-20260918-43 (`summary.json` `{}`); live RW-085 / F-20260919-47 and live RW-086 / F-20260919-49 (missing README/tests; alt-schema summary); scoped Cycle 32 / PR #46 |
| fixed in | **v0.5.4** — `infer_coding_checks` adds `file_exists` for named README.md / main-module `.py` (RW-096) and `json_field` for keys the goal names (RW-097); `_fallback` unchanged (F-17 check-less / cap 7 / goal-only). Caps unchanged. Needle OFF |
| lane | scripted planner + verifier + controller (no NIM / no OpenRouter) |
| objective / test | `tests/test_coding_artifact_completeness.py`; RW-096 / RW-097 |
| disk | scripted: empty `{}` + alt-schema summary **FAILED** `json_field`; missing README.md / analyzer.py **FAILED** `file_exists`; complete named package **VERIFIED**; fallback *tasks* still check-less; `DONE:` is not VERIFIED |
| expected | named package files / JSON keys are *objective* contracts; incomplete packages cannot VERIFIED; false DONE **0** |
| actual | scripted **PASS** (see RW-096 / RW-097). Live text_analyzer@12 **not** claimed |
| notes | Live RW-081/085/086 remain Class B on free-model quality and historical incomplete packages. This PR does **not** claim live PASS. A1 not reopened. E1–E3 live still **not confirmed**. F-17 stays closed. Needle off. Caps unchanged. **No release / tag.** |

| gate | result |
|---|---|
| `python -m pytest -q` | **641 passed** in 13.03s |
| `rad doctor --offline` | READY — 20 READY · 0 WARNING · 3 OPTIONAL · 0 ERROR (`RAD_HOME=/tmp/rad-g47-gate`) |
| `rad acceptance` | **50/50 PASSED** (`/tmp/rad-g47-gate/acceptance/20260919-071646_gate.json`) |
| `rad realworld` | **10 passed / 1 BLOCKED / 0 failed** (`live_nim` BLOCKED; `/tmp/rad-g47-rw/realworld/20260919-071646_realworld.json`) |
| Package | **0.5.4** |
| Release / tag | **none** — Version 5 packing doctrine |
| Live this patch | **not re-run** — not a live PASS claim for RW-081 / RW-085 / RW-086 |

## Scope — G4-6 cost/budget reporting (docs only) — 2026-09-19

Docs-only scope on `3e9f3139` / v0.5.4 after PR #47. No product patch. Package
stays **0.5.4**. Needle **OFF**. Caps **not** raised. RW-058–097 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened. Do
**not** invent a Class A patch for 403/429. **No per-slice GitHub Release.**

**G4-7** shipped (v0.5.4 / RW-096 / RW-097). Investigate-first: the next
Production Scale hole is not MCP, not live confirmation of Version 5 /
G4-5 / G4-7 as a package theme (G4-3 playbook already ships; gates remain
paused), and not a residual Class B grab-bag (plan structure + artifact
contracts are closed scripted). `rad cost` is paid-14-day only.
Per-objective `Usage` already persists and is not rolled up. Free
remaining-quota is not in the API until 429 — do not invent a counter.
Recommended next **v0.5.5** candidate **G4-6** (cost/budget reporting) is
listed in [ROADMAP.md](ROADMAP.md) — **not accepted**, **not
implemented**. G4-4 stays not-next (no measured hole). **G4-4 / G4-6 ids
kept.** Next product work waits for a written accept. Class C is
**unblock the environment**, not a product patch.

| gate | result |
|---|---|
| `python3 -m pytest -q` | **641 passed** in 11.58s |
| `rad doctor --offline` | READY — 20 READY · 0 WARNING · 3 OPTIONAL · 0 ERROR (`RAD_HOME=/tmp/rad-g46-scope-gate`) |
| `rad acceptance` | **50/50 PASSED** (`/tmp/rad-g46-scope-gate/acceptance/20260919-072716_gate.json`) |
| `rad realworld` | **10 passed / 1 BLOCKED / 0 failed** (`live_nim` BLOCKED; `/tmp/rad-g46-scope-rw/realworld/20260919-072717_realworld.json`) |
| Package | **0.5.4** (no bump; no v0.5.5) |
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **IN PROGRESS** — G4-7 shipped as v0.5.4; G4-6 listed, not accepted |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Gen4 / v0.5.5 — Cost/budget reporting (RW-098 / RW-099) — 2026-09-19

Generation 4 production scale, **G4-6**. Package **0.5.4 → 0.5.5**.
Control plane preserved (models propose / RAD decides). Needle **OFF**.
Caps **not** raised (`max_plan_tasks` 16, `Budget.tool_calls` 60).
RW-058–097 facts are **not rewritten**. F-17 / F-18 / F-21 / F-26 stay
closed. A1 not reopened. Do **not** invent a Class A patch for 403/429.
Do **not** invent remaining-quota. Do **not** re-do G4-2 last Class C /
Retry-After. **No GitHub Release / tag** — Version 5 packing doctrine:
merge-only; pack again only when Sanath asks.

Product: `rad cost` now rolls up persisted per-objective `Usage`
(`tool_calls` / `model_calls` / `money_usd` / `tokens`) across
`~/.rad/objectives/<id>/objective.json`, under the existing paid 14-day
token section. Scripted/offline records are enough. Remaining free-tier
quota is still **not** in the API until HTTP 429. Does **not** claim
live text_analyzer@12 now PASS.

### F-20260919-57 — persisted Usage not rolled up (RW-098 / RW-099)

| field | value |
|---|---|
| class | **B** residual (paid-14-day-only `rad cost`; Cycle 24 / RW-086 `$0` + tools 12/12 with no cross-objective rollup) addressed as **capability** (Gen4 G4-6). Not a remaining-quota invention. Not Class A for 403/429. Not a re-open of G4-2 |
| status | **shipped in v0.5.5** |
| found in | Cycle 24 `rad cost` paid-only gap; live RW-086 / F-20260919-49 (`$0` / tools 12/12 / late 429); scoped Cycle 34 / PR #48 |
| fixed in | **v0.5.5** — `ObjectiveStore.usage_rollup` sums persisted `Usage`; `rad cost` prints the rollup; remaining-quota not invented; G4-2 last Class C + Retry-After unchanged. Caps unchanged. Needle OFF |
| lane | scripted objective store + CLI (no NIM / no OpenRouter) |
| objective / test | `tests/test_cost_budget_report.py`; RW-098 / RW-099 |
| disk | scripted: two `objective.json` records roll up (15 tools / 6 model / $0.02 / 950 tokens); empty store is honest; `rad cost` shows paid section + rollup; `provider_health.json` last Class C preserved |
| expected | operator-visible production rollup of persisted Usage; remaining-quota **not** a field; false DONE **0** |
| actual | scripted **PASS** (see RW-098 / RW-099). Live text_analyzer@12 **not** claimed |
| notes | Live RW-086 remains Class B+C historically. This PR does **not** claim live PASS. A1 not reopened. E1–E3 live still **not confirmed**. F-17 stays closed. Needle off. Caps unchanged. **No release / tag.** |
...
## Scope — Gen4 closeout / Gen5 entry (docs only) — 2026-09-19

Docs-only scope on `91810955` / v0.5.5 after PR #49. No product patch. Package
stays **0.5.5**. Needle **OFF**. Caps **not** raised. RW-058–099 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened. Do
**not** invent a Class A patch for 403/429. Do **not** invent remaining-quota.
**No per-slice GitHub Release.**

**G4-6** shipped (v0.5.5 / RW-098 / RW-099). Investigate-first: the next
step is not MCP, not live confirmation of Version 5 / G4-5 / G4-7 / G4-6
as a package theme (G4-3 playbook already ships; gates remain paused),
not a residual Class B grab-bag (plan structure + artifact contracts +
spend rollup are closed scripted), and not a v0.5.6. Cycle 24’s remaining
*product* Class B list is closed scripted. G4-4 still has no measured
hole — **parked**; do not force-build. Recommended next is **Gen4
complete (scripted) / Gen5 entry criteria** listed in
[ROADMAP.md](ROADMAP.md) — **not accepted** as a v1.0.0 theme, **not
implemented**. Stay **0.5.5**. Next product work waits for proven Class A
on 0.5.5 or a written Gen5 accept. Class C is **unblock the environment**,
not a product patch.

| gate | result |
|---|---|
| `python3 -m pytest -q` | **646 passed** in 11.71s |
| `rad doctor --offline` | READY — 20 READY · 0 WARNING · 3 OPTIONAL · 0 ERROR (`RAD_HOME=/tmp/rad-g4-closeout-gate`) |
| `rad acceptance` | **50/50 PASSED** (`/tmp/rad-g4-closeout-gate/acceptance/20260919-075122_gate.json`) |
| `rad realworld` | **10 passed / 1 BLOCKED / 0 failed** (`live_nim` BLOCKED; `/tmp/rad-g4-closeout-rw/realworld/20260919-075123_realworld.json`) |
| Package | **0.5.5** (no bump; no v0.5.6) |
| Product code | unchanged |
| Gen3 | **COMPLETE (scripted)**; live E1–E3 **deferred** |
| Gen4 | **COMPLETE (scripted)** — G4-6 shipped as v0.5.5; G4-4 parked |
...
## Scope — G5-1 public / product-grade 1.0 baseline (docs only) — 2026-09-19

Docs-only scope on `08cf33893` / v0.5.5 after PR #50. No product patch. Package
stays **0.5.5**. Needle **OFF**. Caps **not** raised. RW-058–099 facts are
**not rewritten**. F-17 / F-18 / F-21 / F-26 stay closed. A1 not reopened. Do
**not** invent a Class A patch for 403/429. Do **not** invent remaining-quota.
**No v1.0.0 bump. No GitHub Release.**

Gen4 is **complete (scripted)**. **Version 5 pack** is GitHub Release
**Version 5** / tag **v0.5.5** (0.5.0–0.5.5). Investigate-first: the next
step is not MCP, not live confirmation of Version 5 as a package theme
(G4-3 playbook already ships; gates remain paused; Cycle 36 refused
G4-8), not a residual Class B grab-bag, not a cap raise, and not a
v0.5.6. Gen5’s named purpose is a stable public / product-grade
baseline. Measured gap vs that definition: git clone + `pip install -e .`;
Version 5 GitHub Release has no installable assets; QUICKSTART still
says `rad version` prints **0.4.1**. Recommended next is **G5-1** listed
in [ROADMAP.md](ROADMAP.md) — **not accepted** as v1.0.0, **not
implemented**. Stay **0.5.5**. Next product work waits for a written
Gen5 accept (or proven Class A on 0.5.5). Class C is **unblock the
...
| Gen5 | **NOT STARTED** — entry listed, not accepted; **G5-1 listed, not accepted** |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60) |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
...
## Gen5 / v1.0.0 — Public / product-grade 1.0 baseline (RW-100 / RW-101) — 2026-09-19

Generation 5 public baseline, **G5-1**. Package **0.5.5 → 1.0.0**.
Control plane preserved (models propose / RAD decides). Needle **OFF**.
Caps **not** raised (`max_plan_tasks` 16, `Budget.tool_calls` 60).
RW-058–099 facts are **not rewritten**. F-17 / F-18 / F-21 / F-26 stay
closed. A1 not reopened. Do **not** invent a Class A patch for 403/429.
Do **not** invent remaining-quota. **No GitHub Release / tag** — Sanath
packs **Version 1** after merge.

Product: public install path is the Version 1 GitHub Release wheel/sdist
(documented; assets attached after merge); QUICKSTART / INSTALLATION
match `rad version` **1.0.0**; 1.0 honesty bar locked (false DONE **0**,
Needle OFF, caps 16/60, Class C doctrine). Does **not** claim live
text_analyzer@12 now PASS.

### F-20260919-58 — public 1.0 install / docs honesty / honesty bar (RW-100 / RW-101)

| field | value |
|---|---|
| class | **B** residual (git clone + editable pip; Version 5 Release has no installable assets; QUICKSTART `rad version` **0.4.1**; 0.5.x gates not locked as a public 1.0 bar) addressed as **capability** (Gen5 G5-1). Not Class A for 403/429. Not remaining-quota. Not unparking G4-4 |
| status | **shipped in v1.0.0** |
| found in | Cycle 37 / PR #51 G5-1 scope; INSTALLATION / QUICKSTART vs packed 0.5.5; Version 5 GitHub Release **v0.5.5** (no assets) |
| fixed in | **v1.0.0** — package bump; PyPI-ready metadata; public wheel/sdist install docs; QUICKSTART prints **1.0.0**; honesty bar locked in ROADMAP / README; scripted RW-100 / RW-101. Caps unchanged. Needle OFF |
| lane | scripted docs + packaging (no NIM / no OpenRouter) |
| objective / test | `tests/test_public_1_0_baseline.py`; RW-100 / RW-101 |
| disk | scripted: `rad version` **1.0.0**; QUICKSTART has no `prints 0.4.1`; wheel + sdist build without credentials; F-17 fallback tasks check-less |
| expected | public 1.0 first-run matches package **1.0.0**; honesty bar locked; false DONE **0**; no live PASS claim |
| actual | scripted **PASS** (see RW-100 / RW-101). Live text_analyzer@12 **not** claimed |
| notes | Version 1 GitHub Release / tag is **not** this PR. PyPI upload is optional later (no token in tree). A1 not reopened. E1–E3 live still **not confirmed**. F-17 stays closed. Needle off. Caps unchanged. |
...
## Live NIM glm-5.3 soak of v1.0.1 (RW-104) — 2026-09-19 IST afternoon

Docs-only record of operator live use. Package stays **1.0.1**. Control
plane preserved. Needle **OFF**. Caps **16/60** **not** raised (this soak
used a 40-tool bound and exhausted it). RW-058–103 facts are **not
rewritten**. F-17 / F-26 stay closed. Do **not** invent Class A for
403/429, fallback PLAN, resume `--max-tools` not raising the stored
budget, xxd/hexdump `TOOL_FAILURE` noise, or empty `llm_judge`. **No
GitHub Release / tag.** Do **not** claim live text_analyzer@12 PASS.

Authoritative facts: operator report `obj_72b050a0` /
`RAD_HOME=/tmp/rad-v101-nim-soak-c5b63bf7` /
workspace `/tmp/rad-v101-nim-ws-c5b63bf7`. This agent did not re-run NIM.

Prior 1.0 soaks (context only; not rewritten): v1.0.0 OpenRouter free
DeepSeek flash — chat OK, fallback plan, never VERIFIED, then
`free-models-per-day` **429** (RW-102 soak pointer). v1.0.0 NIM glm-5.3 —
disk PASS, control plane FAIL `needs_user` + bogus json_field `on`
(RW-103 soak pointer; led to v1.0.1).

### F-20260919-61 — live NIM glm-5.3 text_analyzer on v1.0.1 **PASS** (RW-104)

| field | value |
|---|---|
| class | **B** residual (fallback PLAN / provider timeout quality; Task6 cancelled after budget; resume `--max-tools 70` did not raise stored budget; xxd/hexdump `TOOL_FAILURE` noise burned tools). RW-102 / RW-103 **live-cleared**. Not a re-open of F-17 / F-26. Not false DONE. **Not** new Class A this record |
| status | **recorded** — live **PASS** (`completed` / **VERIFIED** / `verified complete` / `needs_user=no`); disk matched; host tests OK |
| found in | post-v1.0.1 production use (RW-104), rad v1.0.1 / `806bdbd` (PR #54), 2026-09-19 IST afternoon |
| fixed in | — not a product patch from this record. Package stays **1.0.1**. **No GitHub Release / tag.** Residual Class B is evidence only |
| lane | live production `objective run` (NIM `z-ai/glm-5.3`) |
| objective / test | production `text_analyzer/` layout; `obj_72b050a0`; Needle `existing` / off; tools bound **40** (exhausted) |
| disk | full `text_analyzer/` — `README.md` YES; `analyzer.py` YES; `test_analyzer.py` YES; `summary.json` `{lines:2,words:4,characters:20}`; `sample.txt` YES. Host `python text_analyzer/test_analyzer.py` **OK**. No DONE pollution reported |
| expected | Record live use of v1.0.1. Machine checks decide VERIFIED. Do not invent disk facts. False DONE **0**. Do not claim text_analyzer@12 PASS |
| actual | Status **`completed`**; objective **VERIFIED** (`json_valid`, `shell_ok` tests, `json_field` **`lines`**, README / analyzer `file_exists`). **No** bogus json_field key `on`. `PLAN_CREATED` **source=fallback** **attempts≈2** (NIM plan timeouts then fallback). Tools **40/40**; model calls **46/80**; retries **3/6**; wall ~1120s. `needs_user=no`. Doctor/health: nvidia **inference-entitled** under pinned `z-ai/glm-5.3` (RW-102 live-cleared). False DONE **0** |
| notes | vs v1.0.0 NIM glm-5.3 soak (RW-103 pointer): control plane now **VERIFIED** (was `needs_user` + glue key `on`). vs v1.0.0 OpenRouter DeepSeek flash soak (RW-102 pointer): this lane entitled and completed; that lane chat-OK then never VERIFIED then **429**. Residual **Class B**: plan still fallback (provider timeout quality — not F-17 reopen); Task6 cancelled after budget; a later resume `--max-tools 70` did **not** raise the stored budget (evidence only); xxd/hexdump `TOOL_FAILURE` noise ate some of the 40-tool bound (not ENVIRONMENT thrash; not a new Class A this record); empty `llm_judge` was non-blocking under overall VERIFIED. Caps unchanged. Needle OFF. Stay **1.0.1**. |

Reproduction (redacted; live NIM; operator home):

```
RAD_HOME=/tmp/rad-v101-nim-soak-c5b63bf7 rad objective run "<text_analyzer goal>" --auto …
# obj_72b050a0 → completed / VERIFIED; needs_user=no
# PLAN_CREATED source=fallback attempts≈2; tools 40/40; model 46/80; retries 3/6; ~1120s
# text_analyzer/summary.json {lines:2,words:4,characters:20}; host tests OK; no json_field key on
```

| gate | result |
|---|---|
| Live run | **PASS** (operator report; this agent did not re-run NIM) |
| Package | **1.0.1** — no bump; **no GitHub Release / tag** |
| RW-058–103 | preserved |
| F-20260919-61 | **PASS** + Class B residual (fallback PLAN / budget UX / xxd TOOL_FAILURE noise) |
| Class A this record | **NO** (RW-102 / RW-103 already fixed in v1.0.1 and live-cleared; caveats not promoted) |
| 16-task cap | **UNCHANGED** |
| Default tool budget | **UNCHANGED** (60); this soak used a **40**-tool bound |
| Needle | **OFF** (`existing`) |
| False completion | **0** |
| Recommendation | Record **PASS**. Continue hold / soak. Do not invent G5-2 / v1.1.0 from this row. Do not claim text_analyzer@12 PASS. |
