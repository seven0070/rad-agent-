...
HTTP 403). OpenRouter free **recovered live** on **v1.0.1** (key in vault;
`default_model` moved from `meta-llama/llama-3.3-70b-instruct:free` — HTTP
404, removed from free tier — to **`liquid/lfm-2.5-2.6b:free`**, HTTP 200,
content_len 39, ts 2026-09-22T17:13:33). Live NIM `z-ai/glm-5.3` on **v1.0.1** is
...
| This change | **RAD Desktop 0.2.0-alpha — distributable product** on package **1.0.1**. 10-page cockpit (Jerry, Objectives, Active Run, Tasks, Trace, Verification, Artifacts, Authority, Memory, Settings) over 9 new desktop API endpoints; Tauri shell owns the bundled `rad-backend` sidecar lifecycle (start/health/reconnect/restart/stale-process, loopback-only, fixed argv, token 0600); UNRESTRICTED explicit-authorization semantics; `sidecar/` PyInstaller pipeline + `.github/workflows/desktop.yml` (tests → frontend → sidecar freeze+smoke → Tauri → installers). **Not** a control-plane rewrite. **783 tests green** incl. authority matrix (5 profiles), desktop security, golden E2E (→VERIFIED→close→reopen), restart/resume (no re-runs), failure/recovery (never fake VERIFIED). Sidecar CLI functionally verified; Rust compile + installers + clean-machine pass are **CI/machine-owned** (no Rust toolchain in this workspace) — not claimed as verified here. Caps **16/60**. Needle **OFF**. **No GitHub Release / tag.** Not **v1.1.0** |
