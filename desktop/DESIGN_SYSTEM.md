...
Conventions for the RAD Desktop app (`desktop`). Read this before
...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-rad` + a `--stroke-rad` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-rad`,
...
```css
shadow-rad           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-rad) /* currentColor hairline, theme-adaptive */
```
...
renderer and Electron's/Tauri's first window paint.
...
| `--stroke-rad` | the overlay hairline (pairs with `shadow-rad`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `rad-mark` on a white tile, softly rounded, identical in light/dark.
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-rad`, `--stroke-rad`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-rad` + `border-(--stroke-rad)`, no hard border?
